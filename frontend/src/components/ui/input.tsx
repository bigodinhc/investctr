...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  ({ className, type, isNumeric, error, leftIcon, rightIcon, ...props }, ref) => {
    const inputClasses = cn(
      "flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm transition-all duration-200 file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-foreground-dim focus-visible:outline-none focus-visible:border-foreground-muted focus-visible:ring-2 focus-visible:ring-foreground-muted/30 disabled:cursor-not-allowed disabled:opacity-50",
